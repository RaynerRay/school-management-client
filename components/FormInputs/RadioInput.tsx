...
export default function RadioInput({
  ...
  name,
  errors,
  defaultValue, // Add defaultValue to props
}: RadioInputProps) {
  // If defaultValue is not provided, use the first option's id
            ...
            >
              <div className="flex items-center ps-3">
                <input
                  {...register(`${name}`, {
                    required: true,
                    value: defaultSelected, // Set default value here
                  })}
                  name={`${name}`}
                  id={item.id}
                  type="radio"
                  value={item.id}
                  defaultChecked={item.id === defaultSelected} // Add defaultChecked
